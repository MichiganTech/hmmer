...
MSAExpand(
  ...
  for (size_t i = msa->nseqalloc-msa->nseqlump; i < msa->nseqalloc; i++){
      msa->sqname[i] = NULL;
      msa->wgt[i]    = -1.0;
   ...
   */
  if (msa->gs != NULL)
    for (size_t i = 0; i < msa->ngs; i++){
      if (msa->gs[i] != NULL){
        msa->gs[i] = ReallocOrDie(msa->gs[i], sizeof(char *) * msa->nseqalloc);
        for (size_t j = msa->nseqalloc-msa->nseqlump; j < msa->nseqalloc; j++)
          msa->gs[i][j] = NULL;
      }
    }
...
MSASetSeqAccession(
  ...
  if (msa->sqacc == NULL) {
    ...
    for (size_t x = 0; x < msa->nseqalloc; x++)
      msa->sqacc[x] = NULL;
  }
  msa->sqacc[seqidx] = strdup(acc);
}
...
MSAAddComment(
  ...
  msa->comment[msa->ncomment] = strdup(s);
  msa->ncomment++;
  return;
}
...
MSAAddGF(MSA *msa, char *tag, char *value)
{
  ...
  msa->gf_tag[msa->ngf] = strdup(tag);
  msa->gf[msa->ngf]     = strdup(value);
  msa->ngf++;
...
MSAAddGS(MSA *msa, char *tag, int sqidx, char *value)
{
  ...
  if (msa->gs_tag == NULL){	/* first tag? init w/ malloc  */
    ...
    for (size_t i = 0; i < msa->nseqalloc; i++)
    	msa->gs[0][i] = NULL;
  }else {
		/* new tag? */
...
MSAAppendGC(MSA *msa, char *tag, char *value)
{
  ...
  if (tagidx == msa->ngc) {
    msa->gc_tag[tagidx] = strdup(tag);
    msa->ngc++;
  }
  char *tmp = realloc(&(msa->gc[tagidx]), strlen(msa->gc[tagidx]) + strlen(value) + 1);
  if(NULL == tmp)
    Die("Allocation failure");
  //&(msa->gc[tagidx]) = strcat(&(msa->gc[tagidx]), value);
  msa->gc[tagidx] = strcat(msa->gc[tagidx], value);
  return;
}
...
MSAAppendGR(MSA *msa, char *tag, int sqidx, char *value)
{
  ...
  if(NULL == tmp)
    Die("Allocation failure");
  //&(msa->gr[tagidx][sqidx]) = strcat(tmp, value);
  msa->gr[tagidx][sqidx] = strcat(tmp, value);
  return;
}
...
{
  if (!getline(&(afp->buf), &(afp->buflen), afp->f))
    return NULL;
  afp->linenumber++;
  return afp->buf;
}
...
MSAFromAINFO(char **aseq, AINFO *ainfo)
{
  ...
  for (i = 0; i < ainfo->nseq; i++)
    {
      ...
      msa->sqname[i] = strdup(ainfo->sqinfo[i].name);
  ...
  msa->name    = ainfo->name ? strdup(ainfo->name) : NULL;
  msa->desc    = ainfo->desc ? strdup(ainfo->desc) : NULL;
  msa->acc     = ainfo->acc  ? strdup(ainfo->acc)  : NULL;
  msa->au      = ainfo->au   ? strdup(ainfo->au)   : NULL;
  msa->ss_cons = ainfo->cs   ? strdup(ainfo->cs)   : NULL;
  msa->rf      = ainfo->rf   ? strdup(ainfo->rf)   : NULL;
...
MSASmallerAlignment(MSA *msa, int *useme, MSA **ret_new)
{
  ...
  for (oidx = 0; oidx < msa->nseq; oidx++)
    if (useme[oidx])
      {
	new->aseq[nidx]   = strdup(msa->aseq[oidx]);
	new->sqname[nidx] = strdup(msa->sqname[oidx]);
  ...
  new->desc    = strdup(msa->desc);
  new->acc     = strdup(msa->acc);
  new->au      = strdup(msa->au);
  new->ss_cons = strdup(msa->ss_cons);
  new->sa_cons = strdup(msa->sa_cons);
  new->rf      = strdup(msa->rf);
  for (i = 0; i < MSA_MAXCUTOFFS; i++) {
