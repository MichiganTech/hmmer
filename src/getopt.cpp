...
Getopt(
  ...
  if (opt[opti].argtype != sqdARG_NONE) {
    ...
    if (opt[opti].argtype == sqdARG_INT && 0 != scanf(*ret_optarg, "%d", tmpInt)){
    	Die("Option %s requires an integer argument\n%s", opt[opti].name, usage);
    }else if (opt[opti].argtype == sqdARG_FLOAT && 0 != scanf(*ret_optarg, "%f", tmpFloat)){
...
main(
  ...
  while (Getopt(argc, argv, OPTIONS, NOPTIONS, "Usage/help here", &optind, &optname, &optarg)){
