...
WriteMSF(
   ...
   *****************************************************************/ 
   ...
   for (idx = 0; idx < msa->nseq; idx++)
     {
       gcg_aseq[idx]   = strdup(msa->aseq[idx]);
       gcg_sqname[idx] = strdup(msa->sqname[idx]);
     }
  ...
  for (idx = 0; idx < msa->nseq; idx++)
    {
      fprintf(fp, " Name: %-*.*s  Len:  %5ld  Check: %4d  Weight: %.2f\n",
        namelen, namelen,
        gcg_sqname[idx],
        msa->alen,
